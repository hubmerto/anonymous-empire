#!/usr/bin/env python3
"""
Rebuild archive from @guestsixonetwo list.
Every entry gets added. Discogs is ONLY used for cover art.
Metadata comes directly from the source list.
"""
...
ARTIST_SCENES = {
    ...
    "voicesfromthelake": "Italy", "leiras": "Spain",
...
def parse_entry(line):
    ...
    if m2:
        ...
        rest = rest.strip()
    # Remove "episode ..." remix info for title
...
def search_cover(query):
    """Search Discogs ONLY for a cover image. Returns (id, thumb_url) or None."""
    url = (
    ...
    try:
        ...
        with urlopen(req, timeout=15) as resp:
            ...
            for r in results:
                thumb = r.get("thumb", "")
                if thumb:
                    return r.get("id", 0), thumb, r.get("catno", "")
    except:
        pass
    return None
...
def main():
    ...
    for i, entry in enumerate(entries):
        ...
        # Search Discogs for cover art only
        ...
        discogs_url = ""
        catno = ""
        discogs_id = i + 10000  # fallback ID

        if cover:
            discogs_id, thumb_url, catno = cover
            dest = IMAGES_DIR / f"{discogs_id}.jpg"
            if dest.exists() or download_image(thumb_url, dest):
                image_path = f"images/{discogs_id}.jpg"
        ...
        if (i + 1) % 50 == 0:
            print(f"  [{i+1}/{len(entries)}] {covers_found} covers, {no_cover} no cover")
            _save(data)

    # Dedup by cover hash (remove duplicate images)
    hash_seen = set()
    no_dupe = []
    for r in data["releases"]:
        ...
        if full and full.exists():
            ...
            if h in hash_seen:
                r["image"] = ""  # keep entry, remove dupe image
            else:
                hash_seen.add(h)
        no_dupe.append(r)
    ...
    print(f"\n{'='*60}")
    print(f"Done: {len(data['releases'])} releases")
    print(f"Covers found: {covers_found}, No cover: {no_cover}")
